...
class RiskManager:
    ...
    def manage_trailing_stop(self, side: str, entry_price: float, current_price: float, current_sl: float, atr: float):
        try:
            trail_offset = 0.6 * atr
            trail_stop = 0.9 * atr

            if side == "BUY":
                trigger_price = entry_price + trail_offset
                if current_price > trigger_price:
                    new_sl = current_price - trail_stop
                    if new_sl > current_sl:
                        log_info(f"[TRAIL] OFFSET atteint (LONG). Nouveau SL proposé: {new_sl:.2f}")
                        return new_sl
                ...
                if current_price < trigger_price:
                    new_sl = current_price + trail_stop
                    if new_sl < current_sl:
                        log_info(f"[TRAIL] OFFSET atteint (SHORT). Nouveau SL proposé: {new_sl:.2f}")
                        return new_sl

        except Exception as e:
            log_error(f"[TRAIL] Erreur gestion trailing: {e}")

        return current_sl  # Aucun changement
