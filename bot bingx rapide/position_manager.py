...
from memory import save_recent_win  # ✅ Pour mémoire persistante des profits
...
class PositionManager:
    ...
    def open_trade(self, side: str, qty: float, tp: float = None, sl: float = None):
        try:
            ...
            if response.get("success", False):
                ...
                self.entry_price = self.api.get_price(self.symbol)  # ✅ Sauvegarde du prix d’entrée
    ...
    def close_trade(self, side: str):
        try:
            ...
            # ✅ Calcul du profit & enregistrement
            ...
            if self.entry_price and close_price:
                ...
                save_recent_win(profit)  # Sauvegarde dans memory.json
