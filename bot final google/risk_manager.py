...
class RiskManager:
    def __init__(
        ...
        logging.info(f"[RiskManager INIT] Capital initial: {self.initial_capital} | Max Drawdown: {self.max_dd} USDT")
    ...
    def calculate_qty(self, equity: float) -> float:
        ...
        qty = round(qty, 3)
        logging.info(f"[RiskManager] Taille de position calculée: {qty} | Équity: {equity}")
        return qty
    ...
    def add_trade_result(self, profit: float):
        ...
        if profit < 0:
            ...
            logging.warning(f"[RiskManager] Perte ajoutée: {abs(profit)} | Perte journalière: {self.daily_loss}")
    ...
    def is_blocked(self) -> bool:
        ...
        if blocked:
            logging.error(f"[RiskManager] 🚫 Blocage du trading activé. Perte journalière: {self.daily_loss} > Max autorisé: {self.max_dd}")
        return blocked
    ...
    def _reset_if_new_day(self):
        ...
        if today != self.last_reset_date:
            logging.info("[RiskManager] Nouvelle journée détectée, réinitialisation des pertes.")
            self.daily_loss = 0.0
